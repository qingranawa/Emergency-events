...
- 小波取消
- 正常大波之间最短间隔：5 分钟
- 第一波积分高的一方优先；平局 50/50 随机
- 后续波次按双方 Support Score 比例随机选择 NTF/CI
- 每次实际支援周期结束后保留 25% 积分，采用四舍五入规则
...
6. 当前保证 SCP 数量至少为两只时包含两只 SCP-939。
...
- 第一波高分阵营优先，平局随机。
- 后续正常大波按 Support Score 比例选择阵营。
- 所有 mini wave 在选择、刷新和刷新完成阶段防御性取消。
- 正常波次至少间隔 300 秒。
- 不调用 Respawn.ForceWave，不主动加速原版时间。
